...
elco_irms_import_csv <- function(files) {
  ...
  d_metadata_analysis <-
    purrr::map2_df(d, seq_along(d), function(x, i) {
      tibble::tibble(
        file_id = i,
  ...
  d <-
    purrr::map(d, function(x) {
      dplyr::mutate(x,
                    ...
                    "sample_mass" = quantities::set_quantities(as.numeric(.data$sample_mass), unit = "mg_sample", errors = 0),
                    ...
                    "N" = quantities::set_quantities(as.numeric(.data$N)/100, unit = "g_N/g_sample", errors = 0),
                    "C" = quantities::set_quantities(as.numeric(.data$C)/100, unit = "g_C/g_sample", errors = 0)
      )
    })

  # add file number
  d <-
    purrr::map2(d, seq_along(d), function(x, i) {
      dplyr::bind_cols(
        tibble::tibble(
