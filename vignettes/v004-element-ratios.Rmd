...
3. Compute the nominal oxidation state of carbon (C$_\textrm{ox}$), the ocidative ratio (OR), and the degree of unsaturation.
...
d <- 
  d %>%
  purrr::map_df(quantities::set_quantities, unit = "g/g", errors = 0) %>%
  purrr::map2_df(colnames(d), function(x, y) elco::elco_new_elco(x, el_symbol = y))

# add sample mass
d <- 
  d %>%
  dplyr::mutate(sample_mass = quantities::set_quantities(
...
Functions `elco_nosc`, `elco_or`, and `elco_du` enable the computation of the C$_\textrm{ox}$, OR, and degree of unsaturation, respectively.
