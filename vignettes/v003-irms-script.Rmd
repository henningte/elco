...
# data import
d <- 
  list.files("./../inst/extdata/irms/", full.names = TRUE) %>% # "./../data/raw_data/irms/"
...
If the signal area for the samples differs clearly from those of the standards, especially BBOT_CN., it is very likely that there is a bias in the measured isotope signatures and element contents.

```{r checks-signal, fig.width=6, fig.height=5, fig.cap="Signal areas of the EA-IRMS chromatograms for all samples and the different standards. Numbered rows represent different imported files and columns contain the data for the two different peaks (<sup>13</sup>C and <sup>15</sup>N).", echo=FALSE}
d %>%
  dplyr::mutate(
...
```{r checks-isotopes-standards, fig.width=6, fig.height=5, fig.cap="Isotope signatures [&delta;&#8240;] for all standards. Numbered rows represent different imported files and columns contain the data for the two different isotopes (<sup>13</sup>C and <sup>15</sup>N). The red points are the respective known true isotope signatures for each standard and isotope.", echo=FALSE}
  ...
  geom_boxplot() +
  geom_point(data = elco::irms_standards %>%
...
```{r checks-isotopes-standards-time, fig.width=6, fig.height=5, fig.cap="Isotope signatures [&delta;&#8240;] for all standards over the time of measurement. Different panels show the data for different isotopes (<sup>13</sup>C and <sup>15</sup>N) and different files (indicated by numbers). Points are the measured values and solid lines are lines of a fitted regression model (to make trends more visible). Different colours represent different standards. Dashed horizontal lines represent the known true isotope signatures of the standards.", echo=FALSE}
  ...
  geom_hline(data = elco::irms_standards %>%
               dplyr::rename(sample_label = standard_name) %>% 
               tidyr::pivot_longer(cols = dplyr::all_of(c("13C", "15N")),
                                   names_to = "isotope",
                                   values_to = "isotope_signature"), 
...
d %<>%
  elco::elco_irms_correct_isotopes(isotope = "13C", 
                                   ref = elco::irms_standards[irms_standards$standard_name == "BBOT_CN.", ],
